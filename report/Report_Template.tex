...
database filtering, RAG context retrieval, and recommendation generation.
...
filters and RAG context retrieval using vector embeddings. This architecture improves control
...
Online shopping platforms contain millions of products, where product inventory gets updated 
frequently. In this project, we targeted to build a lightweight shopping assistant that can help users to find products from an inventory database given a natural-language query.
...
limitations. In our exploratory testing indicated in notebook \texttt{product\_search/baselines/baseline\_llm\_query\_generator.ipynb}, when asking a general-purpose model
for products such as ``a mouse under \$50,'' some returned recommendations
could include related accessories such as mouse pads rather than mouse products as requested.

To address this issue, we propose a hybrid approach that combines a fine-tuned lightweight language model for product category classification, with structured database filtering and RAG context retrieval, so that the final recommendations understands the user request semantically while also satisfying strict constraints.

The main generative model problem investigated in this project is whether a
small language model can be fine-tuned to accurately map user product requests into a
fixed taxonomy of 16 product categories for downstream database filtering. We compare three model families as potential candidates for fine-tuning a lightweight classifier that has close parameter size, and evaluate their performance by classification accuracy, macro F1, weighted F1, and other metrics. The best-performing model is then integrated into the final shopping assistant pipeline.
